...
function testClick(id) {
  ...
  // manually trigger the sortBy column id for ascending
  tree.children[0].children[0].children[id].props.onClick();
  // re-rendering
  tree = component.toJSON();
  expect(tree).toMatchSnapshot();

  // manually trigger the sortBy column id for descending
  tree.children[0].children[0].children[id].props.onClick();
  // re-rendering
  tree = component.toJSON();
  expect(tree).toMatchSnapshot();
...
test('Sort products by decathlon_id, first click is ascending mode, second is descending', () => {
  testClick(0);
});

test('Sort products by title, first click is ascending mode, second is descending', () => {
  testClick(1);
});

test('Sort products by price, first click is ascending mode, second is descending', () => {
  testClick(2);
});
